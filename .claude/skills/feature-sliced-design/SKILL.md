...
`views/`, and `app/`. Add `features/`, `entities/` only when they
provide clear value. Do not create empty layer folders "just in case."
This project intentionally omits the `widgets/` layer.

This project uses 5 layers (no `widgets/`) with a strict top-down import direction:

```text
app/       → App initialization, providers, routing
views/     → Route-level composition, owns its own logic (this project uses "views" instead of standard "pages")
...
import { useUser } from "@/entities/user"; // views → entities
...
- Used in only one view → keep it in that `views/` slice.
- Used in 2+ views but duplication is manageable → keeping separate copies
  in each view is also valid.
- An entity or feature used in only one view → keep it in that view
...
| --------------------- | ------------------------------------------- | ------------------------------------- |
| User profile form     | `views/profile/ui/ProfileForm.tsx`          | `features/profile-form/`              |
| Product card          | `views/products/ui/ProductCard.tsx`         | `entities/product/ui/ProductCard.tsx` |
| Product data fetching | `views/product-detail/api/fetch-product.ts` | `entities/product/api/`               |
| Auth token/session    | `shared/auth/` (always)                     | `shared/auth/` (always)               |
| Auth login form       | `views/login/ui/LoginForm.tsx`              | `features/auth/`                      |
...
| Modal content         | `views/[view]/ui/SomeModal.tsx`             | —                                     |
...
forbidden. (`widgets` layer is not used in this project.)
...
### 5-1. Views First — place code where it is used

Place code in `views/` first. Extract to lower layers only when truly needed.
When extraction seems worthwhile, discuss with the team — this is a design
decision that affects the whole project.

**What stays in views:**

- Large UI blocks used only in one view
- View-specific forms, validation, data fetching, state management
- View-specific business logic and API integrations
- Code that looks reusable but is simpler to keep local

**Evolution pattern:** Start with everything in `views/profile/`. When another
view needs the same user data and the team agrees, extract the shared model to
`entities/user/`. Keep view-specific API calls and UI in the view.
...
```text
// ✅ This project's layer setup
src/
  app/         ← Providers, routing
  views/       ← All view-level code (custom layer name for "pages")
  shared/      ← UI kit, utils, API client

// Add layers only when the team decides they are needed:
// + features/  ← User interactions reused in 2+ views
// + entities/  ← Domain models reused in 2+ views/features
// ❌ widgets/  ← Not used in this project
```
...
- **Be cautious adding UI to entities.** Entity UI tempts cross-imports from
  ...
  higher layers (features, views) — never from other entities.
...
1. **Merge slices** — If two slices always change together, they likely belong
   in one slice.
2. **Extract shared logic to entities** — If multiple features share
   domain logic, move that logic to `entities/`. Keep UI in features.
...
- **Import direction**: `app → views → features → entities → shared` (`widgets` not used)
- **This project's layers**: `app/` + `views/` + `features/` + `entities/` + `shared/`
- **Create entities when**: 2+ views/features share the same business
  domain model, and the team agrees.
- **Create features when**: 2+ views share the same user interaction,
  and the team agrees.
