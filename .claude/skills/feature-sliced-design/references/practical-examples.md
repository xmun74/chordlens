...
is: what goes in `shared/` vs. what goes in `features/` or `views/`?
...
is genuinely consumed by 2+ views/features for **non-auth purposes** (e.g.,
...
| Types used only within one page                   | `views/[name]/model/[name].ts`                               |
...
```typescript
// views/product-detail/api/fetch-product.ts
...
// views/products/api/products-api.ts
...
```typescript
// views/dashboard/api/dashboard-queries.ts
