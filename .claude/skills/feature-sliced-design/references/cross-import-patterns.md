...
When multiple features share the same domain logic, extract that
...
// Solution: views/post composes both and passes data down.
...
// views/dashboard/model/setup.ts — wire dependencies here
...
4. **Only use @x between entities.** Features and views should use
...
entities/notification/
  model/notification.ts ← Used only in views/dashboard
